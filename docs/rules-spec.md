...
**Cyclic paths.** A closed loop has no output end. Rule: find the empty cell with the lowest `(y, x)`; begin resolution at the cell immediately upstream of it and walk backwards around the loop, visiting every cell **except the empty one itself** — that is `n − 1` visits, exactly one per item. Visiting the empty cell too would move the item that just arrived there a second time, advancing one item two cells in a tick; the loop must rotate by exactly one. If **no** cell in the loop is empty, the loop does not move this tick — a legal deadlock. Note that closed loops cannot arise in normal play — under §4's connection rule no external building can ever feed one — but the resolver must handle them because the test harness constructs seeded states (§13, §14 case 9). `ASSUMPTION`
